...
const ReportPage: React.FC<ReportPageProps> = ({ meals, challenges, onClose }) => {
  ...
  const weeklyStats = useMemo(() => {
    ...
    // FIX: Explicitly type the accumulator in `reduce` to ensure `foodCounts` has the correct type Record<string, number>.
    // This resolves the downstream error in `.sort()` where TypeScript couldn't infer that the values being compared were numbers.
